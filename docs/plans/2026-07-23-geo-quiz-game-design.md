...
- **Recognizability signal — three different signals, because no single one works for everything:**
  - **Entities (people, landmarks, dishes): Wikipedia PAGEVIEWS** (annual en.wikipedia views), with per-category floors below which they're dropped — **people & nationality ≥150K, landmarks/dishes ≥35K** (those categories get far fewer views, so a lower floor) — and log-normalized fame for difficulty. Pageviews have a huge dynamic range here (Garbure 21K vs Sophocles 535K vs Einstein 5M+), so they cleanly separate famous from obscure.
  - **Two person clue types, deliberately different:** `birthplace` ("where was X *born*?") is a gotcha (Hitler→Austria, Musk→South Africa) → floored at medium; `nationality` ("which country is X *from*?") uses **single-citizenship** P27 (multi-citizenship dropped as ambiguous) → the easy, no-gotcha association (Gandhi→India, Mandela→South Africa).
  - **Pageviews overrate athletes** (fan/match traffic ≠ general fame), so a low `sitelinks ≥ 30` PRE-filter (bound candidates) + pageviews floor (150K, inclusion+difficulty) is combined with a **`sitelinks ≥ 120` gate to qualify as *easy*** for person questions. Sitelinks is the better household-name signal (icons ~280+; mid-tier footballers far lower), so easy person questions are encyclopedic icons (Descartes, Dalí, Pelé, Turing) while mid-tier athletes (Guðjohnsen, Wanyama) correctly sit in medium/hard. Pelé stays easy (icon, high sitelinks) — the gate keys on fame, not "is-athlete."
  - **Countries (locate/flag/capital): population + a curated whitelist.** Pageviews and sitelinks both FAIL for countries — they're compressed (Serbia 2.6M ≈ Uzbekistan 2.7M; every country is well-documented), measuring topic-curiosity, not flag/geo recognizability. Population (log-normalized) orders the obscure end well, but populous-but-obscure nations (Sudan, Myanmar, Uganda) still leak into "easy" — so the **easy tier is gated by a curated `EASY_COUNTRIES` whitelist** (~45 globally recognizable countries) in `packages`/content-gen. This is what quiz games do; no metric substitutes for it.
...
**Current output:** ~**242-day** calendar (each day has a bonus). Dropping the sitelinks pre-filter (30) + pageviews grew the pool to a 400-day cap, but the `sitelinks ≥ 120` easy-gate (to keep footballers out of easy) trims easy back to ~242 — the deliberate quality/quantity trade (easy tier = icons only; medium/hard are deep). Regenerate with `pnpm content:gen`. Pageviews cached in `tools/content-gen/.cache/pv/` (~6.3K).
